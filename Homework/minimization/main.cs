...
public static (vector, matrix) central(System.Func<vector,double>f, vector x){
    ...
    var points = new List<(double, double)> {(10, 10)};
    vector xx = new vector(2);
        foreach ((double p1, double p2) point in points) {
            xx[0] = point.p1; xx[1] = point.p2;
            vector solution = newton(Himmel_grad, xx, acc:1e-10).Item1;
            vector solution2 = newton(Himmel_grad, xx, acc:1e-10, central_ax:true).Item1;
            WriteLine($"        Computed root with start {(point.p1, point.p2)} and acc:1e-10: {(solution[0], solution[1])}");
            WriteLine($"        Number of newton steps: {newton(Himmel_grad, xx, acc:1e-10).Item2}");
            WriteLine($"        Computed root with start {(point.p1, point.p2)} and acc:1e-10 and central difference approximation: {(solution2[0], solution2[1])}");
            WriteLine($"        Number of newton steps: {newton(Himmel_grad, xx, acc:1e-10, central_ax:true).Item2}");
        }
    WriteLine();
    ...
    WriteLine("     - Computed constants for the Breit-Wigner fucntion with initial guess (126, 2, 10): ");
