...
class main{
    static double integrate(System.Func<double,double> f, double a, double b,
    ...
    static double erf(double z, double accuracy=0.001, double epsilon=0.001){
        if(z<0){
            ...
            return 2.0/Sqrt(PI)*integrate(F, 0, z, acc:accuracy, eps:epsilon );
        }
        else{
            System.Func<double,double> F = (t) => Exp(-Pow(z+(1-t)/t, 2))/t/t;
            return 1-2.0/Sqrt(PI)*integrate(F, 0, 1, acc:accuracy, eps:epsilon);
        }
    }
    ...
    public static int Main(string[] args){
        ...
        WriteLine($"    * ∫_0^1 dx √(x) = 2/3. Is integrate(√(x), 0, 1)=2/3 with acc=0.001 and eps=0.001? {approx(integrate(f1, 0, 1), 2.0/3.0, acc:0.001, eps:0.001)}");
        System.Func<double,double> f2 = (x) => 1.0/Sqrt(x);
        WriteLine($"    * ∫_0^1 dx 1/√(x) = 2. Is integrate(1/√(x), 0, 1)=2 with acc=0.001 and eps=0.001? {approx(integrate(f2, 0, 1), 2.0, acc:0.001, eps:0.001)}");
        System.Func<double,double> f3 = (x) => 4.0*Sqrt(1-Pow(x,2));
        WriteLine($"    * ∫_0^1 dx 4*√(1-x^2) = π. Is integrate(4*√(1-x^2), 0, 1)=π with acc=0.001 and eps=0.001? {approx(integrate(f3, 0, 1), PI, acc:0.001, eps:0.001)}");
        System.Func<double,double> f4 = (x) => Log(x)/Sqrt(x);
        WriteLine($"    * ∫_0^1 dx ln(x)/√(x) = -4. Is integrate(ln(x)/√(x), 0, 1)=-4 with acc=0.001 and eps=0.001? {approx(integrate(f4, 0, 1), -4.0, acc:0.001, eps:0.001)}");
        ...
        WriteLine($"    - Computed value using implemented Clenshaw-Curtis: {CC_var(f_b1, 0.0, 1.0)}, integrand evaluations: {ncalls}");
        ncalls=0;
        WriteLine($"    - Computed value using only 4-point open adaptive quadrature: {integrate(f_b1, 0.0, 1.0)}, integrand evaluations: {ncalls} ");
        ...
        WriteLine($"    - Computed value using implemented Clenshaw-Curtis: {CC_var(f_b2, 0.0, 1.0)}, integrand evaluations: {ncalls}");
        ncalls=0;
        WriteLine($"    - Computed value using only 4-point open adaptive quadrature: {integrate(f_b2, 0.0, 1.0)}, integrand evaluations: {ncalls} ");
        ...
        WriteLine($"    - Computed value using implemented integrator: {integrate(f_b3, 0.0, double.PositiveInfinity)}, integrand evaluations: {ncalls}");
