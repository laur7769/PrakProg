...
//End of ODE routine
    ...
    public static int Main(string[] args)
    {
        ...
        WriteLine($"       Is {newton(f1, new vector(1.0))[0]} = 2 within given accuracy? {vector.approx(newton(f1, new vector(1.0))[0], 2.0, acc: 1e-2)}");
        WriteLine($"       Is {newton(f1, new vector(-1.0))[0]} = -3 within given accuracy? {vector.approx(newton(f1, new vector(-1.0))[0], -3.0, acc: 1e-2)}");
        ...
        vector root1 = new vector(Sqrt(2), Sqrt(2));
        vector root2 = new vector(-Sqrt(2), -Sqrt(2));
        WriteLine($"       {(res1[0], res1[1])} = (√2, √2) within given accuracy? {root1.approx(new vector(res1[0], res1[1]), acc: 1e-2)}");
        WriteLine($"       {(res2[0], res2[1])} = (-√2, -√2) within given accuracy? {root2.approx(new vector(res2[0], res2[1]), acc: 1e-2)}");
        ...
        vector ros_res = newton(Rosen_grad, new vector(7.0, -2.0), acc: 1e-4);
        WriteLine($"        Computed root with start (7.0, -2.0) and acc=1e-4: {(ros_res[0], ros_res[1])}");
        WriteLine($"        Is the computed root equal to the minimum within the given accuracy? {ros_res.approx(new vector(1.0, 1.0), acc: 1e-4)}");
        ...
        WriteLine("        Analytical gradient: ∂f/∂x=4x^2+4xy-42x+2y^2-14,   ∂f/∂y=2x^2-22+4yx+4y^3-26y");
        ...
        foreach ((double p1, double p2) point in points)
        {
            ...
            vector solution = newton(Himmel_grad, xx, acc: 1e-6);
            WriteLine($"        Computed root with start {(point.p1, point.p2)} and acc:1e-6: {(solution[0], solution[1])}");
            WriteLine($"        Is the computed root equal to one of the known minima within the given accuracy? {mini(solution, 1e-6)}");
            WriteLine();
        }

        WriteLine("B. Bound states of hydrogen atom with shooting method for boundary value problems");
        double rmin = 0.01;
        double rmax = 8.0;
        ...
        WriteLine("     From this plot it can be seen that E_0 converges at an rmax around 5.");
        ...
        WriteLine("     Here E_0 is the exact value at rmin=0.01, and E_0 converges to values below the exact value at rmin below 0.01.");
        WriteLine("     Based on these convergence tests, the earlier coice of rmax=8 and rmin=0.01 for computing E_0 should give and accurate value and therfore accurate f_0");
        ...
        WriteLine("     From these plots it can be seen how E_0 converges at an acc and eps of around 0.001.");
        WriteLine("     This indicates that acc and eps probably should have been set to these values in the earlier computation of E_0 instead of the default 0.01 for both ac and eps.");
